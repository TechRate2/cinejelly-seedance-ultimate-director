...
7. Run `validation:billing-admin-ops` and `validation:production-ops` against the HTTPS deployment after the live-input report says those captures can run.
8. Run `npm.cmd run validation:release-audit` and keep the release-audit report with the release evidence.
9. Update readiness notes and remaining blockers in `docs/PROJECT_CONTEXT.md` and `docs/IMPLEMENTATION_ROADMAP.md`.
