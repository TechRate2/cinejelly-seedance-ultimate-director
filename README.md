...
- `validation:quality-benchmark` now supports optional structured semantic-review, audio-review, ASR/lip-sync runtime-review, governance-review, generated-audio validation, and long-form validation JSON checkpoints plus a parity evidence matrix on top of artifact, media probe, sampled-frame, transition-boundary, audio waveform, and duration-sync proxy evidence; it remains no-spend/no-network and never claims full DirectorBench parity or customer-traffic readiness by itself.
