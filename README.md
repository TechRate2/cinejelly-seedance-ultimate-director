...
- `validation:quality-benchmark` now supports optional structured semantic-review and audio-review JSON checkpoints on top of artifact, media probe, sampled-frame, and transition-boundary proxy evidence; it remains no-spend/no-network and never claims full DirectorBench parity or customer-traffic readiness by itself.
