...
- Runtime readiness now has local Atlas credentials, configured model IDs, FFmpeg/FFprobe availability, no-spend readiness, one short paid provider validation, and a trustworthy clean release-candidate Git/source-hygiene audit. Customer release still depends on manual media/artifact/redaction review, optional approved material catalogs or provider keys for source-material fulfillment, optional multimodal LLM validation for source-video auto-analysis, long-form validation, and Atlas generated-audio validation if audio generation is enabled.
