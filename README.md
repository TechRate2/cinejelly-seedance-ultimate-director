...
- Runtime validation currently has real Atlas credentials, verified local FFmpeg/FFprobe access, a 60/60 Phase 6 validation-readiness report, one completed short paid Atlas render, no-spend client policy, deployment-readiness, billing/admin/quota, and production-operations evidence tooling, and a clean release-candidate release audit. Customer use is still blocked by full commercial evidence for a real HTTPS deployment, long-form/source-video/remote-stock/generated-audio paths, billing/admin operations, and production operations.
...
6. Run `npm.cmd run validation:ops-config -- --write-drafts`, fill real non-secret operator attestations and client policies, then run `validation:billing-admin-ops` and `validation:production-ops` against the HTTPS deployment.
7. Run `npm.cmd run validation:release-audit` and keep the release-audit report with the release evidence.
8. Update readiness notes and remaining blockers in `docs/PROJECT_CONTEXT.md` and `docs/IMPLEMENTATION_ROADMAP.md`.
