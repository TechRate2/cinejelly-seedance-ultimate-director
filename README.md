...
6. Run `npm.cmd run validation:ops-config -- --write-drafts`, fill real non-secret operator attestations, run `npm.cmd run ops:promote-attestations`, then run `validation:billing-admin-ops` and `validation:production-ops` against the HTTPS deployment.
