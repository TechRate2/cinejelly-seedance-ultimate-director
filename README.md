...
- Compact async job history now also retains a bounded provider checkpoint summary from cost-ledger entries, and the provider reconciliation foundation can query active prediction IDs through the provider abstraction to produce redacted status evidence without storing raw provider payloads.
