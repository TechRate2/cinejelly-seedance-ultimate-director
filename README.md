...
- Async `POST /v1/render-jobs` now honors optional pre-render review checkpoints: pending or revision-required jobs pause before provider spend, approved review queues the render and reserves quota, rejected jobs terminate, and blocked review evidence remains paused for correction.
