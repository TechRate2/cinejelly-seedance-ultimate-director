...
- Compact async job history now also retains a bounded provider checkpoint summary from cost-ledger entries, so stale active jobs restored after restart can show provider operations, prediction IDs, active/terminal prediction sets, latest provider status, and retry evidence for audit without storing raw provider payloads.
