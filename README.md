...
- Runtime readiness now has local Atlas credentials, configured model IDs, FFmpeg/FFprobe availability, no-spend readiness, one short paid provider validation, source-video auto-analysis validation tooling with an explicit spend gate, and a trustworthy clean release-candidate Git/source-hygiene audit. Customer release still depends on manual media/artifact/redaction review, optional approved material catalogs or provider keys for source-material fulfillment, a live source-video auto-analysis run with a real clean HTTPS source video, long-form validation, and Atlas generated-audio validation if audio generation is enabled.
