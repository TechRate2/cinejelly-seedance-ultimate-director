...
| `DirectorBench` | CineJelly-owned benchmark via `npm.cmd run validation:quality-benchmark`; local FFprobe, sampled-frame, scene-change transition-boundary proxy evidence, optional structured semantic-review JSON, and optional structured audio-review JSON can be included, but full DirectorBench parity is not claimed until archived long-form semantic/audio/VLM/ASR evidence and license review exist. |
