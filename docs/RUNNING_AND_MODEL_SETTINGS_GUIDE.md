...
| `DirectorBench` | CineJelly-owned benchmark via `npm.cmd run validation:quality-benchmark`; local FFprobe, sampled-frame, and scene-change transition-boundary proxy evidence can be included, but full DirectorBench parity is not claimed until semantic/audio/VLM/ASR evidence and license review exist. |
