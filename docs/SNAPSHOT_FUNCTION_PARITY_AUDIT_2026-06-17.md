...
| Source-logic translation foundation | Strong TypeScript foundation with Reference Implementations, source lineage records, local validation for priority modules, and a CineJelly-owned Director-style benchmark foundation with local media probe, sampled-frame proxy evidence, scene-change transition-boundary proxy support, and structured semantic-review checkpoint ingestion. | 84-88% |
| Snapshot feature parity across all upstreams | Mixed. VibeFrame/MoneyPrinterTurbo/Seedance foundations are strongest; VideoAgent/OpenMontage/DirectorBench parity is still partial, but DirectorBench-style evidence now includes local media, transition-boundary proxy signals, and optional structured semantic review instead of artifact evidence only. | 66-71% |
...
- Production TypeScript runtime, provider layer, API, optional compact async job history with stale-active recovery plus provider checkpoint/reconciliation/handoff heartbeat evidence, protected lease-service endpoint validation, HTTPS external lease adapter validation, idempotent provider handoff action-ledger validation, local two-worker handoff validation, production handoff capture runner, Director-style benchmark harness with local media probe/sampled-frame, scene-change transition-boundary proxy evidence, and structured semantic-review checkpoint ingestion, CLI validation commands, schemas, Reference Implementations, source lineage, and operator runbooks.
