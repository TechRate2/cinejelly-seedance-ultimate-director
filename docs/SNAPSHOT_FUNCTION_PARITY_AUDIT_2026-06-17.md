...
| Source-logic translation foundation | Strong TypeScript foundation with Reference Implementations, source lineage records, local validation for priority modules, and a CineJelly-owned Director-style benchmark foundation with local media probe/sampled-frame proxy evidence. | 82-86% |
| Snapshot feature parity across all upstreams | Mixed. VibeFrame/MoneyPrinterTurbo/Seedance foundations are strongest; VideoAgent/OpenMontage/DirectorBench parity is still partial, but DirectorBench-style evidence now includes local media signals instead of artifact evidence only. | 64-69% |
...
| `jiaminchen-1031/DirectorBench` | Checkpoint evaluation for script, visual, audio, cross-modal, stability, transition, and quality. | CineJelly-owned benchmark harness now emits script/video/audio/stability/cross-modal metric evidence, confidence-weighted dimension scores, bottlenecks, JSON report contracts, append-only JSONL history, local FFprobe media metadata, and bounded sampled-frame RGB continuity/lighting proxy signals without copying upstream code. | 55% | Still no true shot-boundary transition analysis, VLM/ASR/lip-sync evaluation, generated-audio listening evidence, or full DirectorBench runtime parity; no-license status still blocks direct code reuse. |
...
| Long-form 2-8 minute workflow | Chunking, admission, budget, artifact, validation runner, and Director-style benchmark foundations exist; local media proxy evidence is implemented, while real paid long-form proof and manual media review remain pending. | 68% |
| Director-style backend benchmark | Benchmark harness now scores existing render evidence across script, video, optional audio, stability, and cross-modal dimensions with bottlenecks, report contracts, FFprobe media metadata, and sampled-frame structural proxy signals. | 58% |
