...
| Source-logic translation foundation | Strong TypeScript foundation with Reference Implementations, source lineage records, local validation for priority modules, and a CineJelly-owned Director-style benchmark foundation with local media probe, sampled-frame proxy evidence, and scene-change transition-boundary proxy support. | 83-87% |
| Snapshot feature parity across all upstreams | Mixed. VibeFrame/MoneyPrinterTurbo/Seedance foundations are strongest; VideoAgent/OpenMontage/DirectorBench parity is still partial, but DirectorBench-style evidence now includes local media and transition-boundary proxy signals instead of artifact evidence only. | 65-70% |
...
| `jiaminchen-1031/DirectorBench` | Checkpoint evaluation for script, visual, audio, cross-modal, stability, transition, and quality. | CineJelly-owned benchmark harness now emits script/video/audio/stability/cross-modal metric evidence, confidence-weighted dimension scores, bottlenecks, JSON report contracts, append-only JSONL history, local FFprobe media metadata, bounded sampled-frame RGB continuity/lighting proxy signals, and FFmpeg scene-change transition-boundary pre/post proxy signals when boundaries are detected, without copying upstream code. | 60% | Still no semantic VLM/ASR/lip-sync evaluation, generated-audio listening evidence, or full DirectorBench runtime parity; current short smoke has no detected scene boundaries at the configured threshold, and no-license status still blocks direct code reuse. |
...
| Long-form 2-8 minute workflow | Chunking, admission, budget, artifact, validation runner, and Director-style benchmark foundations exist; local media and transition-boundary proxy evidence is implemented, while real paid long-form proof and manual media review remain pending. | 69% |
| Director-style backend benchmark | Benchmark harness now scores existing render evidence across script, video, optional audio, stability, and cross-modal dimensions with bottlenecks, report contracts, FFprobe media metadata, sampled-frame structural proxy signals, and scene-change transition-boundary pre/post proxy signals when available. | 62% |
...
- Production TypeScript runtime, provider layer, API, optional compact async job history with stale-active recovery plus provider checkpoint/reconciliation/handoff heartbeat evidence, protected lease-service endpoint validation, HTTPS external lease adapter validation, idempotent provider handoff action-ledger validation, local two-worker handoff validation, production handoff capture runner, Director-style benchmark harness with local media probe/sampled-frame and scene-change transition-boundary proxy evidence, CLI validation commands, schemas, Reference Implementations, source lineage, and operator runbooks.
...
- Business-completion audit that separates API/CLI schema blockers from product-code gaps and reports `canClaimFullSnapshotParity=false` while Web UI, deployed durable queue-backed active provider-work resume/reconciliation/handoff, or full semantic/audio media benchmark gaps remain.
