...
| API and CLI runtime foundation | Implemented across render admission, async jobs, optional compact job-history persistence with stale-active recovery, provider checkpoint, reconciliation, local handoff heartbeat, protected lease-service endpoint, HTTPS external lease adapter heartbeat audit evidence, idempotent provider handoff action-ledger replay evidence, local two-worker protected-route handoff evidence, production HTTPS handoff capture runner, readiness endpoints, provider abstraction, artifacts, quotas, validation commands, and one-command launch doctor orchestration. | 94-96% |
| Source-logic translation foundation | Strong TypeScript foundation with Reference Implementations, source lineage records, local validation for priority modules, and a CineJelly-owned Director-style benchmark foundation with local media probe, sampled-frame proxy evidence, scene-change transition-boundary proxy support, structured semantic-review checkpoint ingestion, and structured audio-review checkpoint ingestion. | 85-89% |
| Snapshot feature parity across all upstreams | Mixed. VibeFrame/MoneyPrinterTurbo/Seedance foundations are strongest; VideoAgent/OpenMontage/DirectorBench parity is still partial, but DirectorBench-style evidence now includes local media, transition-boundary proxy signals, optional structured semantic review, and optional structured audio review instead of artifact evidence only. | 67-72% |
...
- Production TypeScript runtime, provider layer, API, optional compact async job history with stale-active recovery plus provider checkpoint/reconciliation/handoff heartbeat evidence, protected lease-service endpoint validation, HTTPS external lease adapter validation, idempotent provider handoff action-ledger validation, local two-worker handoff validation, production handoff capture runner, Director-style benchmark harness with local media probe/sampled-frame, scene-change transition-boundary proxy evidence, structured semantic-review checkpoint ingestion, structured audio-review checkpoint ingestion, CLI validation commands, schemas, Reference Implementations, source lineage, and operator runbooks.
