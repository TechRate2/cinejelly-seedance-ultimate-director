...
| Source-logic translation foundation | Strong TypeScript foundation with Reference Implementations, source lineage records, local validation for priority modules, and a CineJelly-owned Director-style artifact benchmark foundation. | 80-84% |
| Snapshot feature parity across all upstreams | Mixed. VibeFrame/MoneyPrinterTurbo/Seedance foundations are strongest; VideoAgent/OpenMontage/DirectorBench parity is partial. | 62-67% |
...
| `jiaminchen-1031/DirectorBench` | Checkpoint evaluation for script, visual, audio, cross-modal, stability, transition, and quality. | CineJelly-owned artifact-contract benchmark harness now emits script/video/audio/stability/cross-modal metric evidence, confidence-weighted dimension scores, bottlenecks, JSON report contracts, and append-only JSONL history without copying upstream code. | 45% | Still no frame-level transition analysis, VLM/ASR/lip-sync evaluation, generated-audio listening evidence, or full DirectorBench runtime parity; no-license status still blocks direct code reuse. |
...
| Long-form 2-8 minute workflow | Chunking, admission, budget, artifact, validation runner, and Director-style artifact benchmark foundations exist; real paid long-form proof and media-level benchmark evidence are pending. | 67% |
| Director-style backend benchmark | Artifact-contract harness now scores existing render evidence across script, video, optional audio, stability, and cross-modal dimensions with bottlenecks and report contracts. | 45% |
...
- Business-completion audit that separates API/CLI schema blockers from product-code gaps and reports `canClaimFullSnapshotParity=false` while Web UI, deployed durable queue-backed active provider-work resume/reconciliation/handoff, or full media-level benchmark gaps remain.
