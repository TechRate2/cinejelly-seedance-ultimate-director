...
| Source-logic translation foundation | Strong TypeScript foundation with Reference Implementations, source lineage records, local validation for priority modules, and a CineJelly-owned Director-style benchmark foundation with local media probe, sampled-frame proxy evidence, scene-change transition-boundary proxy support, bounded audio waveform/volume proxy evidence, structured semantic-review checkpoint ingestion, and structured audio-review checkpoint ingestion. | 86-90% |
| Snapshot feature parity across all upstreams | Mixed. VibeFrame/MoneyPrinterTurbo/Seedance foundations are strongest; VideoAgent/OpenMontage/DirectorBench parity is still partial, but DirectorBench-style evidence now includes local media, transition-boundary proxy signals, bounded audio waveform proxy signals, optional structured semantic review, and optional structured audio review instead of artifact evidence only. | 68-73% |
