...
| `harry0703/MoneyPrinterTurbo` | One-input staged pipeline, task progress, material sourcing, subtitles/TTS/BGM, API/CLI/WebUI operations. | One-input render pipeline, stage progress, material sourcing, postproduction planning, generated-audio gates, API/CLI, async jobs, quotas, and artifact evidence. | 75% | WebUI is absent, durable task persistence is limited, and live TTS/BGM/generated-audio validation remains pending. |
...
- Production TypeScript runtime, provider layer, API, CLI validation commands, schemas, Reference Implementations, source lineage, and operator runbooks.
