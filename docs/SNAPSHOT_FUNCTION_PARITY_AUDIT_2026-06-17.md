...
| `harry0703/MoneyPrinterTurbo` | One-input staged pipeline, task progress, material sourcing, subtitles/TTS/BGM, API/CLI/WebUI operations. | One-input render pipeline, stage progress, compact job-history persistence with stale-active recovery plus provider checkpoint/reconciliation/handoff heartbeat evidence, protected lease-service endpoint, HTTPS external lease adapter contract, idempotent provider handoff action-ledger execution replay evidence, local two-worker protected-route handoff evidence, production handoff capture runner, live provider action evidence validator with graph-resume enqueue evidence counting, material sourcing, postproduction planning, generated-audio gates, API/CLI, async jobs, quotas, and artifact evidence. | 92% | WebUI is absent, archived deployed multi-worker active provider-work resume/handoff evidence with live Atlas provider action execution and graph-resume ownership proof is not proven, and live TTS/BGM/generated-audio validation remains pending. |
| `jiaminchen-1031/DirectorBench` | Checkpoint evaluation for script, visual, audio, cross-modal, stability, transition, and quality. | CineJelly-owned benchmark harness now emits script/video/audio/stability/cross-modal metric evidence, confidence-weighted dimension scores, bottlenecks, JSON report contracts, append-only JSONL history, local FFprobe media metadata, bounded sampled-frame RGB continuity/lighting proxy signals, FFmpeg scene-change transition-boundary pre/post proxy signals when boundaries are detected, bounded FFmpeg audio waveform/volume proxy signals, FFprobe audio-video duration-sync proxy signals, optional structured semantic-review, audio-review, ASR/lip-sync runtime-review, governance-review, generated-audio validation, and long-form validation checkpoints, and a contract-validated parity evidence matrix, without copying upstream code. | 88% | Still no automated VLM/ASR/lip-sync analyzers, accepted live generated-audio provider evidence, accepted paid long-form validation evidence, real archived semantic/audio/runtime/governance review for a paid long-form artifact, accepted operator/legal governance packet, or full DirectorBench runtime parity; current short smoke has no detected scene boundaries at the configured threshold, and no-license status still blocks direct code reuse. |
...
- Production TypeScript runtime, provider layer, API, optional compact async job history with stale-active recovery plus provider checkpoint/reconciliation/handoff heartbeat evidence, protected lease-service endpoint validation, HTTPS external lease adapter validation, idempotent provider handoff action-ledger execution validation, local two-worker handoff validation, production handoff capture runner, live provider action evidence validator with graph-resume enqueue evidence counting, Director-style benchmark harness with local media probe/sampled-frame, scene-change transition-boundary proxy evidence, bounded audio waveform/sync proxy evidence, structured semantic-review checkpoint ingestion, structured audio-review checkpoint ingestion, structured ASR/lip-sync runtime-review checkpoint ingestion, structured governance-review checkpoint ingestion, generated-audio validation report ingestion, long-form validation report ingestion, contract-validated benchmark parity evidence matrix, CLI validation commands, schemas, Reference Implementations, source lineage, and operator runbooks.
...
- Business-completion audit that separates API/CLI schema blockers from product-code gaps and reports `canClaimFullSnapshotParity=false` while Web UI, deployed durable queue-backed active provider-work resume/reconciliation/handoff, or full semantic/audio media benchmark evidence gaps remain.
