...
- Persistent first-party short-pipeline chat/review UI and managed product workspace storage. The no-spend backend conversation route, opt-in durable redacted session store, and smoke evidence are implemented; browser-visible UI, managed multi-instance storage/backups, and hosted review controls are still pending.
- Live Product URL-to-Video evidence with a real approved public product URL, accepted product facts, reviewed claims, and media-rights approval. The bounded extractor, API route, no-spend smoke, and report contract are implemented; archived live URL evidence is still pending.
