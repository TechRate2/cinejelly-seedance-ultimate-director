...
- First-party Short create/review shell is now served at `/short/create` and smoke-covered by `validation:short-mvp-ui-contract`; it creates durable no-spend conversation sessions, loads stored-session UI contracts, displays Director/workflow/review/action evidence, and keeps client API keys in browser memory while all `/v1` data routes remain protected.
...
- Customer-grade short-pipeline chat/review UI hardening and managed product workspace storage. The no-spend backend conversation route, opt-in durable redacted session store, first-party create/review shell, and smoke evidence are implemented; hosted review controls, managed multi-instance storage/backups, media playback/library controls, and production UI QA evidence are still pending.
