...
- Live multi-worker active provider-work resume evidence; current handoff support includes local leases, a built-in durable lease-service route, HTTPS external-lease heartbeat smoke evidence, idempotent action-ledger execution replay, local two-worker handoff validation, and a production capture runner, but not archived deployed HA ownership evidence, live Atlas provider action execution, or automatic graph resume.
