...
- Live multi-worker active provider-work resume evidence; current handoff support includes local leases, a built-in durable lease-service route, HTTPS external-lease heartbeat smoke evidence, idempotent action-ledger execution replay, local two-worker handoff validation, a production capture runner, and a live provider action validator that now separates graph-resume enqueue evidence from provider callback evidence, but not archived deployed HA ownership evidence, live Atlas provider action execution evidence, or automatic graph resume.
