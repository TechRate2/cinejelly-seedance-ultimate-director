...
- Media-level DirectorBench-style evidence; current `validation:quality-benchmark` covers artifact-contract scoring, confidence, bottlenecks, skipped audio, and report history, but not frame-boundary transition analysis, visual lighting/fidelity checks, ASR/lip-sync, generated-audio listening evidence, or full upstream runtime parity.
...
- Director-style benchmark harness for no-spend backend quality evidence across script, video, optional audio, stability, and cross-modal artifact-contract checkpoints, with confidence-weighted dimension scores, bottlenecks, schema validation, append-only JSONL history, and explicit `canClaimDirectorBenchParity=false` until media-level evidence exists.
...
- Director-style benchmark reports expose only artifact-kind facts, score/confidence summaries, bottlenecks, limitations, and release-gate status; they do not expose provider payloads, media bytes, local artifact paths, tokens, or claim full DirectorBench parity.
