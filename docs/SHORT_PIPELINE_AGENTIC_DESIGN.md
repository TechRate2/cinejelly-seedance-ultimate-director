...
The render handoff includes this strategy in metadata and prompt text so downstream workers receive more than a generic scene list: they receive the niche, creative mode, trend posture, hook angle, proof strategy, idea seeds, winning concept, reference-pattern guardrails, and scene-level quality instructions.
