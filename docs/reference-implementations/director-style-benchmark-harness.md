...
2. The current harness reads CineJelly paid-render evidence, request evidence, optional manual review text, optional structured semantic review JSON, optional structured audio review JSON, optional structured ASR/lip-sync runtime review JSON, and optional local rendered media.
...
4. Structured semantic review evidence can raise or lower the affected visual/cross-modal checkpoint scores, structured audio review evidence can raise or lower narration/BGM/audio cross-modal checkpoints, structured runtime review evidence can raise or lower ASR transcript-alignment and lip-sync timing checkpoints, and waveform/duration-sync proxy evidence can only provide low-confidence structural audio support when review JSON is absent; all remain bounded checkpoint evidence rather than full DirectorBench runtime parity.
