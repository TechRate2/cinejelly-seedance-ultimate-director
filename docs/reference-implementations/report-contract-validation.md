...
- When `render-provider-live-actions-report.json` exists, report-contract validation requires the validator to be no-spend/no-network, requires summary counts to match execution evidence including `graph_resume_enqueue`/`resume_enqueued` evidence counts, rejects pass reports without `--confirm-live-provider-actions`, passing production handoff evidence, provider-call evidence, resume-polling evidence, terminal-close/manual-audit evidence, or full redaction review, only lets the distributed-resume evidence slice become usable when graph-resume evidence exists, and still keeps `canClaimDistributedResume=false`.
