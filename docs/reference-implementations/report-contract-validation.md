...
- Done: validate optional Director-style structured semantic-review, audio-review, runtime ASR/lip-sync review, and governance-review input packets when those files exist.
...
- When `director-style-benchmark-report.json` exists, report-contract validation includes the artifact-contract benchmark schema, verifies it still reports `canClaimDirectorBenchParity=false`, validates `parityEvidenceMatrix` counts, unique requirement ids, and met/missing evidence consistency, requires the generated-audio provider row to be `met` only when the generated-audio validation report has passed every provider/billing/schema/output/manual-review gate, requires long-form duration/manual-review rows to be `met` only with measured 2-8 minute duration or accepted long-form validation evidence, and requires the license/runtime permission row to be `met` only when the structured governance review has accepted all required checks.
