...
- Done: fail the commercial launch inputs contract when the live provider action evidence input or `validation:provider-live-actions` command is missing.
...
- When `commercial-launch-inputs-report.json` exists, report-contract validation requires `commandPlanAudit.status` to be `pass` with no command-plan issues and requires the live provider action evidence input/command to be present.
