...
- Done: validate optional render-provider live action evidence input and live action validation reports, requiring provider-call evidence counts, production-handoff usability, explicit confirmation, redaction review, and false distributed-resume/customer-release claims to stay internally consistent.
...
- When `render-provider-live-actions-report.json` exists, report-contract validation requires the validator to be no-spend/no-network, requires summary counts to match execution evidence, rejects pass reports without `--confirm-live-provider-actions`, passing production handoff evidence, provider-call evidence, resume-polling evidence, terminal-close/manual-audit evidence, or full redaction review, and still keeps `canClaimDistributedResume=false`.
