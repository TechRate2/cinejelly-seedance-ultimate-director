...
- Done: fail render provider production handoff contracts when usable-evidence flags drift from pass/deployment/completed-operation status or when public operation responses serialize raw lease `jobId` values.
- Done: validate optional render-provider live action evidence input and live action validation reports, requiring provider-call evidence counts, graph-resume enqueue evidence counts, production-handoff usability, explicit confirmation, redaction review, and false distributed-resume/customer-release claims to stay internally consistent.
...
- When `render-provider-production-handoff-report.json` exists, report-contract validation requires no provider/render calls, complete acquire/held/heartbeat/release/post-release/list/active operations, summary counts matching operations, deployment host redaction, no raw lease `jobId` fields in public operation responses, usable-evidence flags only for pass deployment captures, and false distributed-resume/customer-release claims.
- When `render-provider-live-actions-report.json` exists, report-contract validation requires the validator to be no-spend/no-network, requires summary counts to match execution evidence including same-entry `resume_polling` + `graph_resume_enqueue` + `resume_enqueued` graph-resume evidence counts, rejects inconsistent action/providerCallKind/resultStatus combinations, rejects pass reports without `--confirm-live-provider-actions`, passing production handoff evidence, provider-call evidence, resume-polling evidence, terminal-close/manual-audit evidence, or full redaction review, only lets the distributed-resume evidence slice become usable when valid graph-resume evidence exists, and still keeps `canClaimDistributedResume=false`.
