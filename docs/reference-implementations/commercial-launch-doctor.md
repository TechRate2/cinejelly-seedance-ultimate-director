...
9. Run the no-spend accepted review-evidence readiness validator.
10. Run launch intake validation.
11. Run live input validation.
12. Run business validation plan.
13. Run commercial input packet generation.
14. Run business completion audit.
15. Run business readiness audit.
16. Write the launch doctor report.
17. Run report-contract validation.
18. Rerun business completion audit so it reads the fresh report-contract status.
19. Rewrite the launch doctor report with the refreshed completion-audit status.
20. Run report-contract validation one final time, then rewrite the doctor report with the final contract status.
...
The report-contract validator enforces these launch-doctor semantics in addition to JSON schema shape. A launch-doctor report must show the core command sequence, refreshed snapshot parity guardrail evidence, refreshed live provider action evidence status, refreshed quality benchmark command evidence, refreshed quality review-evidence status, final report-contract pass, provider handoff smoke pass/warn snapshot statuses when enabled, explicit skipped provider statuses when provider smokes are disabled, and no stale unexpected command failures when code blockers are zero.
