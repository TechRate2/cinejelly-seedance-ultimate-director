...
For full snapshot/product completeness, the audit also keeps separate product-code gaps visible: no first-party Web UI, no distributed active provider-work resume beyond compact stale-active recovery plus provider checkpoint/reconciliation audit evidence, and no full DirectorBench-style benchmark harness. These gaps should not be hidden by a clean API/CLI schema result.
