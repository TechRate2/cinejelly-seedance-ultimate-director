...
- evidence closure planning that groups remaining blockers into ordered code, scope, operator-prep, deployment, live-provider, source-video, remote-stock, budget, and post-paid review phases, with phase-level required input IDs, env placeholders, operator input files, draft files, report-archive paths, and command guard metadata from the safe handoff manifest
...
10. `evidenceClosurePlan` assigns every blocker to exactly one ordered closure phase, carries phase-level commands, command guards, product-gap references, required operator input IDs, env placeholders, related operator input files, draft/template files, and report archive files, keeps `releaseEvidence=false`, and keeps paid dependencies explicit.
