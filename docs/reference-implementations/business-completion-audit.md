...
5. `productCodeGaps` lists known parity blockers that prevent a 100% upstream/product-completeness claim even when schema/command-plan contracts are passing; gaps that require deployment, paid, live-provider, or manual-review evidence must set `completionRequiresExternalEvidence=true` and `canAutomateNow=false`, while product-scope decisions such as first-party Web UI versus API/CLI-only launch must set `scopeDecisionRequired=true`.
