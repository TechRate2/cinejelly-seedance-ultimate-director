...
- Provider handoff can acquire, heartbeat-renew, retain, and release bounded local leases, expose a deployment-token-protected `/v1/render-provider-handoff-leases/*` service backed by durable storage, call an HTTPS external lease-service adapter, and record redacted idempotent worker action intents around provider reconciliation evidence for terminal-close, continue-polling, held-by-other, and manual-audit decisions; it still requires real multi-worker deployment evidence plus live provider close/cancel/resume execution for distributed deployments.
