...
- Job list status includes queue telemetry plus compact queued, running, succeeded, failed, or canceled summaries with current-stage progress fields, provider-checkpoint availability, compact artifact validation status, and detail-availability flags, including `hasError`, but excludes error detail, full provider checkpoint payload, full validation checks, and the full progress event list.
- Per-job status includes queued, running, succeeded, failed, or canceled state plus retained bounded stage progress events, redacted stack-free error name/message, result, cost ledger, compact provider checkpoint evidence, artifact bundle, and artifact validation checks when available.
...
- Provider handoff can acquire, heartbeat-renew, retain, and release bounded local leases, expose a deployment-token-protected `/v1/render-provider-handoff-leases/*` service backed by durable storage, or call an HTTPS external lease-service adapter around provider reconciliation evidence for terminal-close, continue-polling, held-by-other, and manual-audit decisions; it still requires real multi-worker deployment evidence plus idempotent worker resume logic for distributed deployments.
