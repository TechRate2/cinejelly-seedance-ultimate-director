...
- Queue concurrency, queued/running capacity, and retained history are process-level settings; `/v1/preflight` validates the configured queue capacity before customer traffic, and multi-process deployments should place a durable queue in front of the same Director Agent contract.
