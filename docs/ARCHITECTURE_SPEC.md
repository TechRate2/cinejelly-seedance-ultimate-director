...
- Async job submission enforces a process-level queued/running capacity limit before job records, runtime objects, or provider calls are created; saturated queues return a 503 pressure signal for upstream retry behavior.
- Job list status includes compact queued, running, succeeded, failed, or canceled summaries with detail-availability flags.
...
- Queue concurrency, queued/running capacity, and retained history are process-level settings; multi-process deployments should place a durable queue in front of the same Director Agent contract.
