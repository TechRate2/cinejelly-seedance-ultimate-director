...
- Provider handoff can acquire, retain, and release bounded local leases around provider reconciliation evidence for terminal-close, continue-polling, held-by-other, and manual-audit decisions; it is a local foundation and still requires an external lease backend plus worker heartbeat/resume logic for distributed deployments.
