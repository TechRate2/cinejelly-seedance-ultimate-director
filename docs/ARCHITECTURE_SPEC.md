...
- Provider handoff can acquire, retain, and release bounded local leases or call an HTTPS external lease-service adapter around provider reconciliation evidence for terminal-close, continue-polling, held-by-other, and manual-audit decisions; it still requires a deployed external lease backend plus worker heartbeat/resume logic for distributed deployments.
