...
- Provider reconciliation reports, when present, contain only prediction IDs, provider statuses, terminal/still-active decisions, output URL counts, and redacted errors; they must not include raw provider payloads, output URLs, local paths, secrets, signed URLs, or inline media.
