...
The `validation:launch-doctor` command is the no-spend one-command local audit. It refreshes build, deployment package, optional local smoke, local provider resume/handoff smoke evidence including the digest-only Production Graph resume-state capsule, release audit, Director-style quality benchmark, accepted review-evidence readiness, launch intake, live inputs, business plan, commercial inputs, completion audit, business-readiness, and report-contract evidence into `assets/output_deliverables/business-readiness/commercial-launch-doctor-report.json` plus Markdown. The refreshed quality benchmark, review-evidence readiness, and provider-handoff smoke statuses appear in the doctor snapshot, but benchmark `review_required` or `blocked` status, missing/incomplete review-evidence packets, local handoff smoke passes, and resume-state capsule passes remain product/evidence status; they do not become customer-traffic approval, production deployment proof, or distributed resume parity.
...
`npm.cmd run validation:graph-resume-state` is the no-spend digest-only Production Graph resume-state capsule smoke. It creates and reloads a capsule from a fake graph that intentionally contains unsafe prompt text, output URLs, local paths, bearer/token-like metadata, and provider prediction IDs, then verifies the public report stores only safe counts and hashes while keeping distributed-resume and customer-release claims false.

Run `npm.cmd run validation:provider-live-action-draft` before filling `ops/render-provider-live-actions.json` when you want a no-spend template and checklist for the live callback packet. The generated template is intentionally marked template-only and keeps live-callback booleans false, so it must not be copied directly as evidence and cannot unlock graph-resume or distributed-resume claims. Fill `ops/render-provider-graph-resume-enqueues.json` only after the same deployment worker has actually enqueued graph resume; store SHA-256 digests and safe summaries only, then validate it with `validation:provider-graph-resume`.
