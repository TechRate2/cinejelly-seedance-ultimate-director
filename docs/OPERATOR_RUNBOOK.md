...
The `validation:launch-doctor` command is the no-spend one-command local audit. It refreshes build, deployment package, optional local smoke, local provider resume/handoff smoke evidence including the digest-only Production Graph resume-state capsule plus protected queue-service lifecycle, release audit, Director-style quality benchmark, accepted review-evidence readiness, launch intake, live inputs, business plan, commercial inputs, completion audit, business-readiness, and report-contract evidence into `assets/output_deliverables/business-readiness/commercial-launch-doctor-report.json` plus Markdown. The refreshed quality benchmark, review-evidence readiness, and provider-handoff smoke statuses appear in the doctor snapshot, but benchmark `review_required` or `blocked` status, missing/incomplete review-evidence packets, local handoff smoke passes, and resume-state capsule/queue-service passes remain product/evidence status; they do not become customer-traffic approval, production deployment proof, or distributed resume parity.
