...
`CINEJELLY_MAX_GENERATED_AUDIO_INTENTS` controls how many generated-audio planning requests the API accepts per render request. These intents are recorded as reviewable generated-audio evidence for narration, BGM, ambience, or SFX; CineJelly can plan provider-neutral requests and validate provider results, but it does not call an audio generation provider until verified provider execution wiring passes its own Reference Implementation, live validation, and output review.
