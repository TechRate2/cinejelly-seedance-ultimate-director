...
- Local validation status on 2026-06-16T05:10:22.094Z: `node scripts/run-local-validation-smoke.mjs` passed the current local machine checks with readiness decision `ready_for_paid_validation`: 58 checks total, 58 pass, 0 warn, and 0 fail. Atlas keys, Atlas LLM/media base URLs, LLM model ID, Seedance Standard/Fast model IDs, API auth token, output directory, configured FFmpeg/FFprobe executable paths, and pinned Seedance capability records are present in the local ignored `.env`.
...
- Release audit status on 2026-06-16T05:13:17.299Z: `blocked`. Local smoke, paid-render evidence, paid artifact validation, ignored `.env`, and ignored output evidence pass, but Git metadata reports zero tracked source files plus 3335 pending tracked/untracked source items, so source hygiene, tracked secret scan, and import-boundary evidence cannot be trusted. Customer release still requires a repaired or clean Git worktree, manual artifact/media inspection, and manual redaction review.
