...
- `src/api/server.ts` exposes `/health`, `/v1/preflight`, synchronous `/v1/render`, and async `/v1/render-jobs` submit/list/get endpoints; all JSON responses pass through redaction, render pipeline failures after request normalization write redacted `failure-report.json` plus `cost-ledger.json` artifacts before returning 500, output/work/artifact paths are confined to `CINEJELLY_OUTPUT_DIR` or `assets/output_deliverables`, and long-running jobs can be polled without holding one HTTP request open.
