...
- `src/providers` implements provider-neutral contracts, an Atlas Cloud default provider, async polling, Asset Library operations, error normalization, configurable Seedance capabilities, and cost ledger tracking.
...
- `src/agents` and `src/application` wire intake, story architecture, render-time Asset Library reference resolution for video/audio references, rendering, director orchestration, runtime factory, and deployment preflight.
