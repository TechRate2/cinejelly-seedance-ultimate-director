...
- Local validation status on 2026-06-14T06:15:38.759Z: `npm.cmd run typecheck`, `npm.cmd run build`, `npm.cmd run preflight`, and `npm.cmd run validation:readiness` passed the current local machine checks with readiness decision `review_warnings`: 55 checks total, 54 pass, 1 warn, and 0 fail. Atlas keys, LLM model ID, Seedance Standard/Fast model IDs, API auth token, output directory, and configured FFmpeg/FFprobe executable paths are present in the local ignored `.env`.
- The remaining local readiness warning is `ATLASCLOUD_SEEDANCE_CAPABILITIES_JSON`; preflight falls back to documented default capability assumptions until current Atlas schema/capabilities are pinned.
