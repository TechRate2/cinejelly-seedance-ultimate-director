...
- `Commercial Launch Intake`: no-spend ignored-input validator that gives operators one machine-checkable `ops/commercial-launch-intake.json` contract for clean deployment/source URLs, env var names, budget scope, provider choices, and manual-review policy before live or paid evidence runs.
...
- Commercial launch intake status on 2026-06-17: `npm.cmd run validation:launch-intake` is implemented and no-spend/no-network. It writes a draft intake plus Markdown fill-out packet and validates the ignored `ops/commercial-launch-intake.json` file when present. The current local report is expected to be `missing_intake` until an operator fills the draft with real non-secret deployment/source/budget/provider/manual-review values.
