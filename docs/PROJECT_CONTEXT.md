...
- `src/api/server.ts` exposes `/health`, `/v1/preflight`, and `/v1/render`; all JSON responses pass through redaction, render pipeline failures after request normalization write redacted `failure-report.json` plus `cost-ledger.json` artifacts before returning 500, and output/work/artifact paths are confined to `CINEJELLY_OUTPUT_DIR` or `assets/output_deliverables`.
