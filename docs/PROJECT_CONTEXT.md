...
- `Commercial Launch Intake`: no-spend ignored-input validator that gives operators one machine-checkable `ops/commercial-launch-intake.json` contract for clean deployment/source URLs, env var names, budget scope, provider choices, and manual-review policy before live or paid evidence runs; passing intake values can feed the no-spend live-input and business-plan reports without exposing secrets.
- `Commercial Launch Inputs`: no-spend JSON and Markdown packet that turns the current readiness reports into the exact operator-provided URLs, env placeholders, attestation files, approved budget, live evidence commands, paid validation commands, and manual review tasks still required before commercial launch.
- `Business Completion Audit`: no-spend JSON and Markdown summary that separates remaining code/schema/command-plan blockers from true operator, external-provider, budget, paid-validation, and manual-review blockers while keeping full-parity product-code gaps visible.
- `Commercial Launch Doctor`: no-spend one-command orchestrator that refreshes build, local smoke, release hygiene, launch-input, business-plan, business-readiness, completion, and report-contract evidence into one operator-facing JSON/Markdown report before any paid or live evidence run.
...
- Commercial launch intake status on 2026-06-17: `npm.cmd run validation:launch-intake` is implemented and no-spend/no-network. It writes a draft intake plus Markdown fill-out packet and validates the ignored `ops/commercial-launch-intake.json` file when present. `validation:live-inputs` and `validation:business-plan` now read a passing intake as a secret-free source for missing clean deployment/source URLs, source-video enablement, remote-stock intent, and budget ceiling while leaving explicit CLI/env values authoritative. The current local report is expected to be `missing_intake` until an operator fills the draft with real non-secret deployment/source/budget/provider/manual-review values.
- Commercial launch input status on 2026-06-16: `npm.cmd run validation:commercial-inputs` is implemented and no-spend/no-network. The current packet is `blocked_by_operator_inputs` and writes both `commercial-launch-inputs-report.json` and `commercial-launch-inputs-checklist.md`; it identifies the missing deployment HTTPS URL, billing/admin attestation, production operations attestation, approved Atlas validation budget, source-video URL/enablement, and remote-stock provider choice. It can name the generated-audio smoke as the only currently ready paid Atlas slice when live-inputs confirms the slice-specific billing evidence, while `shouldDeferFullSequenceSpend` and customer traffic release remain false.
- Business completion audit status on 2026-06-17: `npm.cmd run validation:completion-audit` is implemented and no-spend/no-network. It reads the current readiness reports and writes both `business-completion-audit-report.json` and `business-completion-audit.md`; the current local summary confirms no known code/schema/command-plan blocker when contracts pass, while remaining launch blockers are external/operator inputs, budget approval, paid validation, and manual review. It now also emits `productCodeGaps` so first-party Web UI, full distributed active provider-work resume beyond checkpoint/reconciliation/handoff heartbeat plus protected lease-service, HTTPS external lease, idempotent action-ledger, local two-worker handoff, and production handoff capture-runner evidence, and DirectorBench-style benchmark harness gaps remain visible before anyone claims 100% upstream parity.
- Report contract status on 2026-06-17: `node scripts/validate-report-contracts.mjs` passes for current release/business-readiness report contracts, including render provider reconciliation, render provider handoff, render provider external-lease handoff, render provider lease-service smoke, render provider handoff action-ledger smoke, render provider local multi-worker handoff smoke, optional render provider production handoff capture, the generated-audio budget-slice Atlas billing report, commercial launch intake, commercial launch input packet, business completion audit, live-inputs, deployment packaging, billing/admin, production-ops, and generated-audio evidence contracts.
- Commercial launch doctor status on 2026-06-17: `npm.cmd run validation:launch-doctor` is implemented as the one-command no-spend local operator audit. It refreshes build, deployment package, local smoke, release-audit, launch-intake, live-inputs, business-plan, commercial-inputs, completion-audit, business-readiness, and report-contract evidence, then writes `commercial-launch-doctor-report.json` plus Markdown without calling Atlas model endpoints, stock providers, deployment URLs, or source videos.
...
- Behavior-critical upstream parity still needs per-logic Reference Implementations, source lineage records, and validation before CineJelly should claim high-fidelity translation for each new logic area. Phase 1 Prompt Binding Plan, Phase 2 Guardian Repair Decision Provenance, Phase 3 Reference Selection Scoring plus Reference Metadata Enrichment plus Source Video Reference Metadata Enrichment plus Source Video Auto Analysis Adapter plus Source Video Auto Analysis Validation Runner, Phase 4 Provider Polling/Retry/Cost Fidelity, Phase 5 Long-Form Planning/Batch Workflow plus Long-Form Validation Runner plus Render Job Stage Progress Telemetry plus Render Job History Persistence plus Render Provider Reconciliation plus Render Provider Handoff plus Protected Lease Service plus Handoff Action Ledger plus HTTPS External Lease Adapter And Heartbeat plus Local Multi-Worker Handoff Validation plus Production Handoff Capture Runner plus API Artifact Validation Evidence plus Material Source Adapter Validation plus Local Material Library Adapter plus Remote Stock Material Adapter plus Remote Stock Provider Validation Runner plus Postproduction Asset Orchestration plus Generated Audio Intent Planning plus Generated Audio Execution Planner plus Generated Audio Provider Execution Runner plus Generated Audio Output Validation plus Generated Audio Output Batch Validation plus Generated Audio Batch Artifact Evidence including review-packet handoff evidence plus Generated Audio Asset Resolution plus Generated Audio Asset Resolution Catalog plus Generated Audio Provider Execution Contract, Phase 6 Validation Readiness Report plus Phase 6 Render Request Validation Contract plus Phase 6 Paid Render Validation Runner, and Media Tool Binary Resolution foundation now have Reference Implementations, CineJelly rewrites, lineage records, and local typecheck/build validation; real long-form Atlas render validation, the archived live source-video report from a real clean HTTPS source video, the archived live remote stock report from approved provider keys, live paid Atlas generated-audio validation, first-party UI, and deployed multi-worker active-job resume/reconciliation/handoff with live provider action execution remain pending.
