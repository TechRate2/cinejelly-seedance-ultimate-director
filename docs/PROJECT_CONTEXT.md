...
- Atlas billing readiness status on 2026-06-16: `npm.cmd run validation:atlas-billing` is implemented as a no-spend Atlas Billing Public API guard. The local report blocks live network until `--confirm-live-network`; the current business-plan estimate still exceeds the `$5` approval because long-form 120s is estimated at `$24`.
