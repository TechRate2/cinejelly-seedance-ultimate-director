...
- `AudioProvider Boundary`: defines provider-neutral generated-audio capability/request/result contracts; Atlas exposes generated-audio capabilities only when reviewed capability JSON is configured and still fails safely before live audio execution until verified audio schemas, model IDs, pricing, and artifact validation pass.
