...
- `Render Provider Handoff`: wraps provider reconciliation in bounded local file leases so terminal provider work can be closed, still-active work can stay leased for polling, and held-by-other work can defer safely; external durable leases, multi-worker ownership, and automatic graph resume remain separate unfinished work.
