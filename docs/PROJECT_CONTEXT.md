...
- `Render Provider Handoff`: wraps provider reconciliation in bounded local file leases, a deployment-token-protected lease-service route, or an HTTPS external lease-service adapter so terminal provider work can be closed, still-active work can heartbeat-renew its lease for polling, held-by-other work can defer safely, and redacted terminal-close/resume-polling/manual-audit action intents can replay by stable idempotency key; live multi-worker ownership, live provider action execution, and automatic graph resume remain separate unfinished work.
