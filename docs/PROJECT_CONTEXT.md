...
- `Render Provider Handoff`: wraps provider reconciliation in bounded local file leases or an HTTPS external lease-service adapter so terminal provider work can be closed, still-active work can heartbeat-renew its lease for polling, and held-by-other work can defer safely; a deployed durable lease service, multi-worker ownership, and automatic graph resume remain separate unfinished work.
