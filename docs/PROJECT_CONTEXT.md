...
- `Live Readiness Input Validator`: no-spend local gate that checks whether deployment URL/auth, ops attestations, source-video URL/settings, remote stock approvals, generated-audio capability inputs, and long-form budget are ready before any live network or paid Atlas validation.
...
- Report contract status on 2026-06-16: `node scripts/validate-report-contracts.mjs` passes for current release/business-readiness report contracts after fixing generated-audio and validation-runner redaction so boolean/count fields with key-like names remain booleans/counts instead of `[REDACTED]` strings.
