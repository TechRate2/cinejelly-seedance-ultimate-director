...
- CineJelly extension:
  ...
  - Adds a generated-audio provider execution runner that executes only verified ready items, preserves provider-result order, turns provider exceptions into stack-free result evidence, and requires batch output validation before approved tracks can enter assembly.
  ...
  - Adds provider-neutral generated-audio capability/request/result contracts inspired by MoneyPrinterTurbo's explicit audio stage and `/audio` stop-at behavior, while keeping actual paid provider execution behind verified schemas, capability mapping, explicit validation spend, output validation, and manual review.
