...
function buildProductCodeGaps() {
  return [
    ...
    },
    {
      ...
      status: "graph_resume_evidence_contract_ready",
      currentCoveragePercent: 97,
      sourceEvidence: "docs/SNAPSHOT_FUNCTION_PARITY_AUDIT_2026-06-17.md",
      sourcePatternOrigins: ["harry0703/MoneyPrinterTurbo", "vericontext/vibeframe"],
      requiredAction:
        "Run validation:provider-live-action-draft to prepare the operator fill-out checklist, run validation:provider-production-handoff against the real HTTPS deployment lease service, archive the production acquire/held/heartbeat/release handoff evidence, then run a live provider worker on the same deployment that maps persisted action-ledger execution callbacks to real Atlas close/resume/manual-audit behavior and graph_resume_enqueue evidence, and validate the archived packet with validation:provider-live-actions -- --confirm-live-provider-actions. Keep the documented single-process boundary until deployed multi-worker ownership handoff, graph-resume enqueue evidence, and live provider action evidence all pass.",
      canAutomateNow: true,
      blocksApiCliCommercialLaunch: false,
      blocksFullSnapshotParity: true,
      releaseImpact:
        "Provider prediction checkpoints, reconciliation reports, local handoff smoke, HTTPS external lease heartbeat contract evidence, a deployment-token-protected lease-service route, idempotent action-ledger execution replay, local two-worker handoff evidence, launch-doctor evidence refresh, a production handoff capture runner, a non-evidence live provider action template/checklist, and a live provider action plus graph-resume evidence contract improve post-restart auditability. The live action contract now requires consistent action/providerCallKind/resultStatus tuples and a matching production-handoff deployment fingerprint before graph-resume evidence can count, but distributed/HA runtime parity still requires archived real deployment evidence, production multi-worker ownership handoff evidence, live provider action execution, and live Atlas handoff validation."
    },
    {
